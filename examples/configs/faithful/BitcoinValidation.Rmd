...
- This document can be reproduced from preexisting data via "knitting" `r toBitRepo("/examples/configs/faithful/BitcoinValidation.Rmd")` that exists in the same directory.
- The analysis requires R-based logAnalysis tools that can be cloned from `r bitcoinRepo`
- The variable `logAnalysisToolPath` needs to be set to the directory of logAnalysis scripts.
...
Configuration file(s): `r paste0(bitcoinRepo,"/examples/configs/faithful/bitcoin.faithful.30node.30sim.XX-XX.properties")` (where `XX` is to be replaced with the Simulation ID)
Workload: `r toBitRepo("/examples/configs/faithful/bitcoin.faithful-workload.csv")`
Data: `r toBitRepo("/examples/results/faithful/")`
...
... and at the time of fetching it (`2024-11-25`), it is `102.29T`. We need to convert this Bitcoin-specific difficulty to CNSim difficulty (search space/success space) using the `BitcoinDIfficulyUtility.java` routine `BTCToCNSIM(double BTCDiff)`. The result seen below is given as a parameter:
```
