...
We want to study bitcoin for 1 hour of simulated operation time.
...
## Seed Management
All experiments use the same network and workload:
```
...
```
At node level, behavior is the same until time `50,000 ms`, where it is expected that transaction # 290 will arrive at the system.
```
node.sampler.seed = {444,222}
node.sampler.updateSeedFlags = {false,true}
node.sampler.seedUpdateTimes = {50000}
```




# Lightweight Bitcoin Simulation 1
